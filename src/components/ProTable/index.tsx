 ...
 */
import { defineComponent, nextTick, onMounted, ref, watch, provide } from "vue"
import {
  Table, Tooltip,
  type TablePaginationConfig, Checkbox, Space, Empty, InputSearch, Card
} from "ant-design-vue"
...
import type { PMProTableProps, TableColumnsType, TableEmit, FilterDropdownProps, PMColumnType, DataInfo } from './types'
...
export default defineComponent<PMProTableProps>(function PMProTable(_, {
    ...
    if (attrs.rowSelection) {
      finallyDataSource.value = val.map((item, idx) => {
        return {
          ...
          rowKeyId: item.id ?? (attrs.isVirtual ? idx : (page.value - 1) * limit.value + idx)
        }
      })
    } else {
      finallyDataSource.value = val.map((item, idx) => ({
  ...
  const requestCall = async (params: Record<string, any>) => {
    ...
    if(attrs?.fileNames){
      data[attrs?.fileNames.current] = page.value
      data[attrs?.fileNames.pageSize] = limit.value
    }else{
      data.page = page.value
      data.limit = limit.value
    }
    const res = request ? 
     await request?.(data).finally(() => loading.value = false) :
  ...
  const paginateCall = (pagination: TablePaginationConfig) => () => {
    ...
    // 说明是 limit 改变了
    if (limit.value !== pagination.limit) {
      // 当前页变为 1
      page.value = 1
    } else {
      page.value = pagination.current || 1
    }
    limit.value = pagination.pageSize || 20
  }
  /**
   * 数据筛选
   */
  ...
  return () => {
    return (
      <>
        ...
        >
          {/* 高级 form 组件 */}
          <div style={{ borderRadius: '0px',padding: '0px 12px',background:'#fff' }} > {
            attrs.search !== false && (
              <PMProForm
                ref={tQProFormRef}
                search={attrs.search}
          ...
          <a-divider style="margin: 0px; background-color: #fff;" />
          </div>
          {/* 表格 */}
          <div ref={tableRef} style={{ padding: '12px 8px 8px 8px', backgroundColor: '#fff' }}>
            <Table
              style={isUserSelect.value ? { userSelect: 'none' } : {}}
              rowKey='id'
              loading={loading.value}
              size={attrs.size || 'middle'}
              ...
              //     // />
              //     // <TableEmpty emptText={initEmptyQuery.value ? '请手动点击查询' : '暂无数据'} />
              //   )
              // }}
