export function formatNumber(num: any, cent: number, isThousand = true) {
 ...
 * isHorizontal 是否带横杠
 */
export function stringDateFormat(data: string, isHorizontal = true) {
  // 变成不带横杠的
  if (!isHorizontal) {
  ...
  if (data !== null && data !== '' && typeof (data) !== 'undefined') {
    const stringData = `${data.substring(0, 4)}-${data.substring(4, 6)}-${data.substring(6, 8)}`;
    return stringData;
  }
}
